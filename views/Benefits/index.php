...
<section class="content">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark"> Benefits Lists </h1>
                </div><!-- /.col -->
                <div class="col-sm-6" align="right">
                    <?= Html::button('<i class="fa fa-plus-square"></i> New Benefit', ['value'=>Url::to(['/benefits/create']),'title'=>'Create New Benefit','class'=>'showModalButton btn bg-olive']) ?>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
    </div>
    <?php if (Yii::$app->session->hasFlash('success-createben')): ?>
        <div class="alert alert-success alert-dismissable">
            <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
            <?= Yii::$app->session->getFlash('success-createben')?>
        </div>
    <?php endif; ?>
    <?php if (Yii::$app->session->hasFlash('failed-createben')): ?>
        <div class="alert alert-warning alert-dismissable">
            <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
            <?= Yii::$app->session->getFlash('failed-createben')?>
        </div>
    <?php endif; ?>
    <div class="container-fluid">
        <div class="row">
